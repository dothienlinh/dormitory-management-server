...
│   │   │   │   ├── room_router.go              # Room routes
│   │   │   │   ├── room_category_router.go     # Room category routes
│   │   │   │   └── contract_router.go          # Contract routes
│   │
...
- **HTTP Handlers (internal/delivery/http/)**: Xử lý HTTP requests/responses:
  - `server.go`: Khởi tạo HTTP server và middleware
  - **Router (internal/delivery/http/router/)**: Cấu hình và quản lý các routes
    - `routers.go`: Setup tổng thể cho tất cả các routes
    - `auth_router.go`: Các routes liên quan đến xác thực
    - `user_router.go`: Các routes liên quan đến người dùng
    - `room_router.go`: Các routes liên quan đến phòng
    - `room_category_router.go`: Các routes liên quan đến loại phòng
    - `contract_router.go`: Các routes liên quan đến hợp đồng
  - **Middleware (internal/delivery/http/middleware/)**: HTTP middleware
    - `auth.go`: Xác thực người dùng
    - `error.go`: Xử lý lỗi
    - `logger.go`: Ghi log
    - `middleware.go`: Factory cho middleware
  - **Handler (internal/delivery/http/handler/)**: Xử lý logic cho các HTTP endpoints
    - `auth_handler.go`: Xử lý đăng ký, đăng nhập
    - `user_handler.go`: Xử lý tác vụ người dùng
    - `room_handler.go`: Xử lý tác vụ quản lý phòng
    - `room_category_handler.go`: Xử lý tác vụ quản lý loại phòng
    - `contract_handler.go`: Xử lý tác vụ quản lý hợp đồng
    - `handlers.go`: Factory cho handlers
