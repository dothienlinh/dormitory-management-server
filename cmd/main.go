...
func main() {
	...
	response := services.NewAPIResponse()

	middleware := middleware.NewMiddleware(dbClient, redisClient, util, logger, response)

	service := services.NewService(dbClient, redisClient, logger, util, response)

	handler := handlers.NewHandler(service, logger, response)
