...
func init() {
	goose.AddMigrationContext(upUpdateOtpTypeEnum, downUpdateOtpTypeEnum)
}

func upUpdateOtpTypeEnum(ctx context.Context, tx *sql.Tx) error {
	query := `
		ALTER TYPE otp_type_enum RENAME TO _otp_type_enum_old;
		
		CREATE TYPE otp_type_enum AS ENUM ('login', 'register', 'reset_password', 'verify_email', 'verify_phone', 'transaction', 'verify_account', 'forgot_password');
...
func downUpdateOtpTypeEnum(ctx context.Context, tx *sql.Tx) error {
	query := `
